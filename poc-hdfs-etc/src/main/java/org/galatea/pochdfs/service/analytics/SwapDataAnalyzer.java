...
public class SwapDataAnalyzer {
   ...
   * @return a dataset of all enriched positions with unpaid cash for the counter party on the
   *     effective date
   */
  public Dataset<Row> getEnrichedPositionsWithUnpaidCash(final String book,
    ...
    log.info("Current state read took {} ms", System.currentTimeMillis()
        - subStartTime);
    ...
    log.info("Completed enriched positions creation in {} ms",
    ...
    Dataset<Row> enrichedPositionsWithUnpaidCash =
        joinEnrichedPositionsAndUnpaidCash(enrichedPositions,
            unpaidCash);
   ...
   * @param book the book
   * @param effectiveDate the effective date
   * @return a dataset of all enriched positions for the book on the effective date
   */
  ...
  private Dataset<Row> createEnrichedPositions(final BookSwapDataState currentState) {
    ...
    Dataset<Row> startOfDatePositions =
        positions.filter(positions.col("position_type").equalTo("S"));
    Dataset<Row> enrichedPositions =
        DATASET_TRANSFORMER.leftJoin(startOfDatePositions, swapContracts,
            "swap_contract_id");
    enrichedPositions =
        DATASET_TRANSFORMER.join(enrichedPositions, counterParties, "counterparty_id");
  ...
  /**
   * Creates an unpaid cash dataset with additional columns that merge the cashflow type with the
   * unpaid cash. For example, an unpaid cash result might have two records for one position because
   * there are two types, INT and DIV. This method would join those two records into one record by
   * creating unpaid_INT and unpaid_DIV columns for the position.
   *
  ...
  private Dataset<Row> normalizeUnpaidCashByType(final Dataset<Row> unpaidCash) {
    ...
    Dataset<Row> normalizedUnpaidCash =
        combineDistributedUnpaidCash(cashFlowTypes, distributedUnpaidCash)
            .distinct();
    return normalizedUnpaidCash;
  }
  ...
  private List<String> getCashFlowTypes(final Dataset<Row> unpaidCash) {
    ...
    Dataset<Row> pivotSet =
        cashFlowTypeRows.withColumn("tempGroupCol", functions.lit(0)).withColumn("tempSumCol",
            functions.lit(0));
  ...
  private Dataset<Row> distributeUnpaidCashByType(final Dataset<Row> unpaidCash,
      final List<String> cashFlowTypes) {
   ...
   *
   * @param cashFlowTypes the list of cash flow types
   * @param distributedUnpaidCash the unpaid cash with unpaid columns for the different cashflow
   *     types
   * @return a normalized unpaid cash dataset
   */
  ...
  private Dataset<Row> joinEnrichedPositionsAndUnpaidCash(final Dataset<Row> enrichedPositions,
    ...
    Dataset<Row> dataset =
        DATASET_TRANSFORMER.getDatasetWithDroppableColumn(enrichedPositions, "swap_contract_id");

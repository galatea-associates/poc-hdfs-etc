package org.galatea.pochdfs.domain;

public class Defaults {

	public static int		CONTRACT_ID			= 12345;
	public static int		COUNTERPARTY_ID		= 200;
	public static int		CASHFLOW_ID			= 1;
	public static double	AMOUNT				= 100;
	public static String	LONG_SHORT			= "Long";
	public static int		INSTRUMENT_ID		= 11;
	public static String	RIC					= "ABC";
	public static String	CURRENCY			= "USD";
	public static String	CASHFLOW_TYPE		= "INT";
	public static int		EFFECTIVE_DATE		= 20190101;
	public static int		PAYDATE				= 20190102;
	public static String	COUNTERPARTY_FIELD1	= "CPField1";
	public static String	POSITION_TYPE		= "S";
	public static int		TD_QUANTITY			= 100;
	public static String	BOOK				= "ABCDE";

}
